...
export class ConfirmOrderCommand implements Command<{ orderId: string }> {
  constructor(
    private readonly orderRepository: OrderRepository,
    private readonly domainEventBus: DomainEventBus,
    private readonly logger: ILogger,
  ) {
    this.logger.setContext(ConfirmOrderCommand.name);
  }

  async execute(request: { orderId: string }): Promise<void> {
    this.logger.log(`Confirming order ${request.orderId}`);
    ...
    this.logger.log(`Order ${request.orderId} confirmed successfully`);
  }
}
